...
5. Deploy `supabase/functions/certificate` using its pinned `deno.json`. Configure the permitted frontend origin in `ALLOWED_ORIGIN`. The function verifies every bearer token with Auth `getUser()`; private issuance RPCs are service-role-only even with gateway JWT verification disabled. Never expose a service key in frontend variables.
